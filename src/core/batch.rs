...
pub async fn batch(
    ...
    // 请求线程池
    let request_handles_arc = Arc::new(Mutex::new(Vec::new()));
    // 接口线程池
    let mut api_handles:Vec<JoinHandle<Result<(), Error>>> = Vec::new();
    // 统计响应大小
    ...
    for (index, endpoint_arc) in api_endpoints_arc.clone().into_iter().enumerate() {
        ...
        let api_handle: JoinHandle<Result<(), Error>> = tokio::spawn(async move {
            // 根据权重算出来每个接口的并发量
            for _ in 0..concurrency_for_endpoint {
                ...
                request_handles_arc_api_clone.lock().await.push(handle);
            }
            Ok(())
        });
        api_handles.push(api_handle);
    }
    ...
    let mut handles_guard = request_handles_arc.lock().await;
    ...
    for result in results {
        match result {
            Ok(Ok(())) => println!("任务成功完成"),
            Ok(Err(e)) => println!("任务执行出错: {:?}", e),
...
mod tests {
    ...
    async fn test_batch() {
        ...
        match batch(20, 30, true, true, endpoints, Option::from(StepOption { increase_step: 5, increase_interval: 2 })).await {
            Ok(r) => {
                println!("{:#?}", r)
            }
            Err(e) => {
